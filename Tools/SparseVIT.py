...
def alter_sparse(x, sparse_size=8):
    # x: [B, C, H, W]
    B, C, H, W = x.shape
    pad_h = (sparse_size - H % sparse_size) % sparse_size
    pad_w = (sparse_size - W % sparse_size) % sparse_size
    if pad_h > 0 or pad_w > 0:
...
def alter_unsparse(x, H, W, Hp, Wp, sparse_size=8):
    # x: [B_blocks, C, block_H, block_W]
    B = int(x.shape[0] // ((Hp // sparse_size) * (Wp // sparse_size)))
    C = x.shape[1]
    x = x.permute(0, 2, 3, 1).contiguous()  # [B_blocks, block_H, block_W, C]
    x = x.view(B, Hp // sparse_size, Wp // sparse_size, sparse_size, sparse_size, C)
    x = x.permute(0, 1, 3, 2, 4, 5).contiguous()  # [B, H_blocks, block_H, W_blocks, block_W, C]
    x = x.view(B, C, Hp, Wp)  # »¹Ô­ÐÎ×´
    x = x[:, :, :H, :W].contiguous()  # È¥µôpadding²¿·Ö
    return x
...
class Sparse_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads, sparse_size=0, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        global layer_scale
        self.ls = use_layer_scale
        self.sparse_size = sparse_size
        if self.ls:
            #global init_value
            print(f"Use layer_scale: {use_layer_scale}, init_values: {layer_scale_init_value}")
    ...
    def forward(self, x):
        ...
        if self.ls:
            ...
            x_attn = x_attn.transpose(1, 2).reshape(x_sparse.shape)
            x_unsparse = alter_unsparse(x_attn, H, W, Hp, Wp, sparse_size=self.sparse_size)
            x_unsparse_flat = x_unsparse.flatten(2).transpose(1, 2)
            ...
            x = x_befor + self.drop_path(x_unsparse_flat)
