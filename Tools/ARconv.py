...
class ARConv(nn.Module):
    ...
    def remove_hooks(self):
        for handle in self.hook_handles:
            handle.remove()  # ç§»é¤é©å­å½æ°
        self.hook_handles.clear()  # æ¸ç©ºå¥æåè¡¨
    ...
    def forward(self, x, epoch, hw_range):
        ...
        if epoch <= 100:
            ...
            N_X = int(torch.div(mean_l, scale, rounding_mode='floor'))
            N_Y = int(torch.div(mean_w, scale, rounding_mode='floor'))
